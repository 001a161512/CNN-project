...
class Base_Layer(object):  
    ...
    def __init__(self, type_):
        ...
        self.kwargs = {} #permet de sauvegarder les options de chaque layer 
...
class ReLU(Base_Layer):
    ...
    def feedforward(X):
        ...
        return np.maximum(0, X) # la fonction Relu
    ...
    def gradient_activation(Img_In, D_Img_Out):
        ...
        return 1.0*(Img_In>0)*D_Img_Out # deriver de la fonction relu: 1 si X>0 0 sinon
...
class Pooling(Base_Layer):
    def __init__(self, **kwargs):
        ...
        if 'pool' in kwargs.keys():
            ...
            #on verifie si les option de pooling sont bien du format (int, int)
        ...
        if 'type_' in kwargs.keys():
            #type_ permait de choisir entre la fontion max, moyenne, sum
            ...
            #par defaut la fonction utilisé est max
            self.type_ = 'Max'
            self.kwargs['type_'] = 'Max'
        
        if self.type_ == 'Max':
            #pour la fonction max on utilise les code par defaut:
            #on parcoure l'image petit a petit
            self.feedforward = self.feedforward_max
            self.gradient_activation = self.gradient_activation_max
            
            
        elif self.type_ == 'Average':
            #pour la fonction averge on peux utiliser la vitesse de calcule de numpy en sommant 
            #toutes les premiere case du pooling en premier, puis toutes les 2eme etc ...
            self.feedforward = self.feedforward_fast
            self.gradient_activation = self.gradient_activation_fast
            
        elif self.type_ == 'Sum':
            #pour la fonction sum on peux utiliser la vitesse de calcule de numpy en sommant 
            #toutes les premiere case du pooling en premier, puis toutes les 2eme etc ...
            self.feedforward = self.feedforward_fast
            self.gradient_activation = self.gradient_activation_fast
    ...
    def feedforward_fast(self, X):
        ...
        if self.padding:
            #si il y a padding on augmente la taille de l'image pour inclure le padding
        ...
        #on cree une image vide qui sera l'image de sortient
        ...
        if self.type_ == 'Average':
            #si average on divise les valeur par la surface de pooling
